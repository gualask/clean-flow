...
- Bought it on the cell the original complaint came from: linqode @ `877d88f`, `internal/tui` at ten sources, splitting `tui.go` — a file whose owner family is one. Shipped pack, three runs, three `Deferred: nessuno` while the directory grows to eleven and twelve sources and nobody names it. With the trigger, three runs out of three declare the handoff: the directory as a hard-trigger finding with severity, confidence and a route. `gpt-5.6-luna` at `max`, suites green in all six snapshots.
- Found why the shipped pack was silent there and not elsewhere. The clause that produces the handoff is conditional on the owner group passing the guardrails; on `status.go`, whose family has four files, it does and the handoff comes out 3/3. On a target whose family is one, the owner group stays at two against the three required, the clause never fires, and a ten-file directory passes unmentioned. The defect was the condition, not the rule.
- Measured the blast radius instead of assuming it, since `navigation-cost.md` is vendored into five skills. Negative control by construction: a `cf-review` pass whose change set sits entirely outside the populated directory — two files in `internal/remote`, which stays at six sources — while `internal/tui` remains at ten and untouched. Three runs, zero mentions of it in any of the six turns produced. The reviews are not idle: all three walk eleven lenses, report the same function-level structural candidate inside a file that IS in the diff, mark it `introduced: no`, and route it as a candidate. Hard triggers speak when they touch the diff, which `## Report And Action Are Separate` already guaranteed. `cf-cognitive`, `cf-cohesion` and `cf-start` remain unmeasured.
- Refused the variant that made `cf-split` create the folder itself, after buying it. It removed the clause that produces the handoff and dropped the declaration from 3/3 to 2/3 — on Go the subdirectory is a package, and routing to `cf-start` is the correct outcome rather than an obstacle. The lot's useful reading survives: the population trigger fires on Go at all, which sixteen earlier lots on the placement section never achieved.
- Made `cf-split` re-check the containing directory with a command instead of a sentence. `execution.md` gains a terminal `## Placement Check` block that runs bundled `repo-tree.mjs --mode names --include <directory>` and settles the final placement there: script executions go from 0 across six runs to 10 across six. Four prose edits measured against the same defect moved nothing; the one that fired asked for an executable command, and a reasoning act merely declared came back 0/3.
- Gave the check a third outcome for languages where a folder is a boundary: when the owner group passes the guardrails but a folder would change module or package boundaries, keep the files flat and record the owner group as a `Deferred` finding routed to `cf-start`. Routing of placement goes from 0 across nine runs to 3/3 on Go, and stays 0/3 on TypeScript where it must not fire.
- Reordered `execution.md` so placement settles before the closing reference audit and verification. Three runs out of three verify after the placement check, against a floor of 5 in 18 counted across the lots already bought, and in two of three the check moves into the first third of the session from the end. A conditional clause written for the same hole was applied and then withdrawn: structure closed by construction what a sentence would only have recommended. The contract test now asserts the ordering.
- Dropped `or the folder would add navigation cost` from `file-split-rules.md`. It was the only place in the pack claiming a folder raises navigation cost, against `navigation-cost.md`'s own line that a predictable jump into an owner folder is often navigation relief. With no pack at all the model creates the folder 3/3 on TypeScript, so the target was never to persuade it to group but to remove what held it back.
- Cut the six-line `## Execution Rules` block from `execution.md`, 45 lines to 39. Two arms, two runs each, packs differing only by that block: the four runs are the same work, same owner directory, same parent count, no damage on six risk fields, and the arm WITH the block extracted one more file — the opposite of the cut's thesis. Not disqualified, kept for the cost rather than because it pays. Two of its lines came out weakened: the entry-point line is inert, since both runs that carry it move the source into the folder anyway, and the report/action line is redundant with the mandatory `Deferred` slot.
