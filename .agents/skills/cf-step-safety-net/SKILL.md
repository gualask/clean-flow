...
description: Establish a behavior lock before structural changes. Use in Phase B after the current work unit is clear and before bounded structural edits.
---
...
- If no brief exists and the scope is not explicit and local, stop before designing checks and route to `cf-start` or `cf-phase-discovery`.
