...
Identify the real problem, reduce scope, collect only useful context, and recommend the smallest credible next step.
...
- Use agents or packaged reconnaissance only when allowed by the current runtime/user; otherwise keep the local pass bounded or ask for authorization when needed.
...
6. Recommend the smallest useful route or decision. [outcomes](references/outcomes.md)
...
2. `cf-simplify`: the user asks whether an area has too many files, unnecessary complexity, overengineering, or whether changing behavior/UX could enable a cleaner simplification.
