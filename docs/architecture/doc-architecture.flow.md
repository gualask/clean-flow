...
1. Start from the user mapping request and any existing `.cflow/architecture.md`.
2. Preflight only the existing architecture artifact, architecture template, and worktree state.
3. Spawn `cflow_architecture_recon` with repository path and mapping request; keep the agent read-only.
4. While the agent runs, avoid duplicating the repository scan locally.
5. Check the returned report against `architecture.template.md` and spot-check only decision-relevant gaps or contradictions.
