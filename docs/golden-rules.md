...
- Budget every `description` against the whole pack, not against itself: the host's initial skills list is capped at about 2% of context, roughly 8,000 characters across all descriptions, and it is truncated when it overflows — which is why the key use case and the trigger words go first. Sum of the eleven shipped descriptions on 2026-08-14: 4,633 characters. Codex documents only `name` and `description`, with no `when_to_use` field, so trigger phrases belong inline; moving them out was measured sending three of three routing prompts to the wrong skill.
