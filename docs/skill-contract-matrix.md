...
| `cf-step-consolidate-seam` | internal | no | conditional | architecture map plus either a mapped consolidate-oriented work unit, or an explicit local, behavior-preserving seam where the artificial boundary is already clear | stop and route to `cf-start` or `cf-phase-fragmentation-map` when the path or seam is not clear enough |
