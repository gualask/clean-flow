...
| `cf-cognitive` | public | yes | yes | repository state plus optional explicit source file target; `.cflow/*` artifacts are not required | discover and select one local file, ask one focused target question if selection is ambiguous, or route to `cf-start` when the request is broader than local file-by-file cleanup |
...
| `cf-cognitive` | not required | optional; ignored by this skill | does not create or update `.cflow/*`; discovers candidates when needed, performs one local file-level cognitive complexity refactor, and reports checks run | yes |
