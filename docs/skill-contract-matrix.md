...
| `cf-file-split` | public | yes | yes | repository state plus one explicit or inferable target source file; `.cflow/*` artifacts are not required | evaluate candidates when the user asks whether to split, execute one scoped file split when explicitly asked, ask one focused target or placement question when ambiguous, or route to `cf-start` when the request is broader than local file-level extraction |
...
| `cf-cognitive` | not required | optional; ignored by this skill | does not create or update `.cflow/*`; discovers up to three justified candidates when needed, performs local file-level cognitive complexity refactors sequentially, reports checks run, and may route file-level split review to `cf-file-split` | yes |
| `cf-file-split` | not required | optional; ignored by this skill | does not create or update `.cflow/*`; evaluates or executes one scoped behavior-preserving file-level split, reports checks run, and audits references when files or symbols move | yes |
